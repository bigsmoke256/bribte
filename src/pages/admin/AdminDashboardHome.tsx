...
import {
  Users, GraduationCap, BarChart3, Bell, Activity,
  BookOpen, CalendarDays, UserPlus, PlusCircle, Megaphone,
} from "lucide-react";
...
export default function AdminDashboardHome() {
  ...
  const [stats, setStats] = useState({
    students: 0, lecturers: 0, courses: 0, enrollments: 0, activeStudents: 0,
  });
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [s, l, c, e, a, students] = await Promise.all([
  ...
  return (
      <div className="space-y-6 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <StatCard title="Announcements" value={recentAnnouncements.length} subtitle="Recent posts" icon={Bell} delay={0.16} />
        </div>
        ...
        <AnimatedCard delay={0.4} className="!p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: "Total Lecturers", value: stats.lecturers, icon: GraduationCap, color: "text-warning" },
            ].map((s, i) => (
